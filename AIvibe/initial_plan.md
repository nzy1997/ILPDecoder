...
Goal: design and implement a Python package for quantum error correction decoding based on Integer Linear Programming (ILP / MILP). The package should act as a PyMatching-like decoder with an ILP backend and a JuMP-style modular solver abstraction.
...
Planned solver backend: fully modular and user-configurable, inspired by JuMP / MOI.
...
Switching solvers should not require rebuilding the decoding logic.

## 5) Public API (PyMatching-like)

Planned public API to resemble PyMatching:
```
